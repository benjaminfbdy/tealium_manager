...
## [Sprint 13] - Cleanup & Simplification

**👔 Vue Métier**:
*   **Focus**: Suppression des fonctionnalités jugées non essentielles pour alléger l'interface et se concentrer sur les cas d'usage principaux (Gestion, Inventaire, Adobe).
*   **Retraits**: "Explorateur de Profil", "Vérificateur de Mapping" et "Comparaison de Profils" (Live vs Latest) ont été retirés du menu.

**⚙️ Vue Technique**:
*   **Nettoyage**: Suppression des imports et des routes dans `main.py`. Les fichiers de vues et contrôleurs associés sont conservés pour l'instant (code mort) mais désactivés.

## [Sprint 14] - Audit & Refactoring (En cours)

**👔 Vue Métier**:
*   **Stabilité**: Consolidation de l'application après une phase de développement rapide. Abandon de la piste R&D Server-Side pour se concentrer sur la robustesse des fonctionnalités existantes.

**⚙️ Vue Technique**:
*   **Audit**: Identification des points de friction (Monolithe `database.py`, logique métier dans les vues).
*   **Nettoyage**: Suppression du code mort lié à la tentative d'inventaire Server-Side.
*   **Refactoring Architecture (Sprint 15)**:
    *   Découpage de `database.py` en repositories dédiés : `repositories/adobe_repo.py` et `repositories/tealium_repo.py`.
    *   Mise à jour de tous les contrôleurs et vues pour utiliser ces nouveaux modules.
    *   `database.py` ne gère plus que la connexion bas niveau et les paramètres globaux.
    *   Cette structure améliore la maintenabilité et prépare le terrain pour de futures évolutions (ex: chiffrement).
